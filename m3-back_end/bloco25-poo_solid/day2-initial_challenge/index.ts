...
class SalesContract implements Contract {
  ...
  describe(): void {
      ...
      this.clauses.forEach((clause) => { console.log(`Clausula: ${clause}`) });
      this.signatories.forEach((sig) => { console.log(`Assinado por: ${sig.name}`) });
      console.log('-------------------\n');
  }
}
...
const lp = new LegalPerson('International Sales SA', '2396815747621862498254987654');
